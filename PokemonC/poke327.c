...
void init_shop()
{
  ...
  clear();
  int ch;
  mvprintw(1,1,"Welcome to the shop!");
...
void init_battle()
{
  ...
  clear();
  int ch;
  mvprintw(1,1,"A challenger appears!");
  ...
  if(c->alive){
    for (i = base; i < 8 + base; i++) {
      if ((world.hiker_dist[c->pos[dim_y] + all_dirs[i & 0x7][dim_y]][c->pos[dim_x] + all_dirs[i & 0x7][dim_x]] <= min))
      {
        ...
        else if(world.cur_map->cmap[c->pos[dim_y] + all_dirs[i & 0x7][dim_y]][c->pos[dim_x] + all_dirs[i & 0x7][dim_x]]->symbol == PC) {
          init_battle();
          c->alive = false;
        }
...
void show_trainers()
{
  clear();
  int i, j, dx, dy, key;
  int px = world.pc.pos[dim_x];
  int py = world.pc.pos[dim_y];

  printw("Current Trainer Locations: \n");


  for(j = 0; j < MAP_Y; j++) {
    for(i = 0; i < MAP_X; i++) {
      if(world.cur_map->cmap[j][i]) {
        if(world.cur_map->cmap[j][i]->symbol == PC) {
        }
        else if(world.cur_map->cmap[j][i]->symbol == HIKER) {
          dx = i - px;
          dy = j - py;
          if(dx < 0) {
            printw("Hiker located: %d to the west & ", abs(dx));
          }
          else {
            printw("Hiker located: %d to the east & ", abs(dx));
          }
          if(dy > 0) {
            printw("%d to the south.\n", abs(dy));
          }
          else {
            printw("%d to the north.\n", abs(dy));
          }
        }
        else if(world.cur_map->cmap[j][i]->symbol == RIVAL) {
          dx = i - px;
          dy = j - py;
          if(dx < 0) {
            printw("Rival located: %d to the west and ", abs(dx));
          }
          else {
            printw("Rival located: %d to the east and ", abs(dx));
          }
          if(dy > 0) {
            printw("%d to the south.\n", abs(dy));
          }
        ...
        else if(world.cur_map->cmap[j][i]->symbol == PACER) {
          dx = i - px;
          dy = j - py;
          if(dx < 0) {
            printw("Pacer located: %d to the west and ", abs(dx));
          }
          else {
            printw("Pacer located: %d to the east and ", abs(dx));
          }
          if(dy > 0) {
            printw("%d to the south.\n", abs(dy));
          }
        ...
        else if(world.cur_map->cmap[j][i]->symbol == WANDERER) {
          dx = i - px;
          dy = j - py;
          if(dx < 0) {
            printw("Wanderer located: %d to the west and ", abs(dx));
          }
          else {
            printw("Wanderer located: %d to the east and ", abs(dx));
          }
          if(dy > 0) {
            printw("%d to the south.\n", abs(dy));
          }
        ...
        else if(world.cur_map->cmap[j][i]->symbol == SENTRY) {
          dx = i - px;
          dy = j - py;
          if(dx < 0) {
            printw("Sentry located: %d to the west and ", abs(dx));
          }
          else {
            printw("Sentry located: %d to the east and ", abs(dx));
          }
          if(dy > 0) {
            printw("%d to the south.\n", abs(dy));
          }
        ...
        else if(world.cur_map->cmap[j][i]->symbol == WALKER) {
          dx = i - px;
          dy = j - py;
          if(dx < 0) {
            printw("Walker located: %d to the west and ", abs(dx));
          }
          else {
            printw("Walker located: %d to the east and ", abs(dx));
          }
          if(dy > 0) {
            printw("%d to the south.\n", abs(dy));
          }
...
{
  /*
  * Can't get window to work properly so just adding three lines.
  */
  int x, y;
    ...
    if (c == &world.pc) {
      ...
      while(turn == 0) {
        ...
        switch (input) {
          ...
          case 121:
          case 55:
            if(move_cost[char_pc][world.cur_map->map[world.pc.pos[dim_y] - 1][world.pc.pos[dim_x] - 1]] != INT_MAX && !world.cur_map->cmap[world.pc.pos[dim_y] - 1][world.pc.pos[dim_x] - 1]) {
            ...
            else if(move_cost[char_pc][world.cur_map->map[world.pc.pos[dim_y] - 1][world.pc.pos[dim_x] - 1]] != INT_MAX && world.cur_map->cmap[world.pc.pos[dim_y] - 1][world.pc.pos[dim_x] - 1]->alive == true) {
              init_battle();
              world.cur_map->cmap[world.pc.pos[dim_y] - 1][world.pc.pos[dim_x] - 1]->alive = false;
            }
                turn = 1;
                pathfind(world.cur_map);
                break;
          case 107:
          case 56:
            if(move_cost[char_pc][world.cur_map->map[world.pc.pos[dim_y] - 1][world.pc.pos[dim_x]]] != INT_MAX && !world.cur_map->cmap[world.pc.pos[dim_y] - 1][world.pc.pos[dim_x]]) {
              world.cur_map->cmap[world.pc.pos[dim_y]][world.pc.pos[dim_x]] = NULL;
              world.pc.pos[dim_y] = world.pc.pos[dim_y]-1;
              world.cur_map->cmap[world.pc.pos[dim_y]][world.pc.pos[dim_x]] = &world.pc;
            }
            else if(move_cost[char_pc][world.cur_map->map[world.pc.pos[dim_y] - 1][world.pc.pos[dim_x]]] != INT_MAX && world.cur_map->cmap[world.pc.pos[dim_y] - 1][world.pc.pos[dim_x]]->alive == true) {
              init_battle();
              world.cur_map->cmap[world.pc.pos[dim_y] - 1][world.pc.pos[dim_x]]->alive = false;
            }
                turn = 1;
                pathfind(world.cur_map);
                break;
          ...
          case 108:
          case 54:
            if(move_cost[char_pc][world.cur_map->map[world.pc.pos[dim_y]][world.pc.pos[dim_x] + 1]] != INT_MAX && !world.cur_map->cmap[world.pc.pos[dim_y]][world.pc.pos[dim_x] + 1]) {
              world.cur_map->cmap[world.pc.pos[dim_y]][world.pc.pos[dim_x]] = NULL;
              world.pc.pos[dim_x] = world.pc.pos[dim_x]+1;
              world.cur_map->cmap[world.pc.pos[dim_y]][world.pc.pos[dim_x]] = &world.pc;
            }
            else if(move_cost[char_pc][world.cur_map->map[world.pc.pos[dim_y]][world.pc.pos[dim_x] + 1]] != INT_MAX && world.cur_map->cmap[world.pc.pos[dim_y]][world.pc.pos[dim_x] + 1]->alive == true) {
              init_battle();
              world.cur_map->cmap[world.pc.pos[dim_y]][world.pc.pos[dim_x] + 1]->alive = false;
            }
                turn = 1;
                pathfind(world.cur_map);
                break;
